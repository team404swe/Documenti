...
\begin{itemize}
		\item\code{+ getQuestionDetails(QMLtext)}: restituisce la domanda corrente se essa è valida;\\
		\item\code{+ deleteQuestion(questionID)}: richiede la rimozione dal database di una domanda;\\
		\item\code{+ openAlertQuestion(questionID)}: ;\\
	\end{itemize}
\end{itemize}

\subsubsection{View::Templates::QuestionController}
\begin{itemize}
...
\subsubsection{View::Templates::QuizListController}
\begin{itemize}
...
\item\textbf{Funzionalità}:
	\begin{itemize}
		...
		\item\code{+ setQuiz(qID}:\\
		\item\code{+ deleteQuiz(quiz)}: permette l'eliminazione del quiz scelto\\
		\item\code{+ openAlertQuiz(quizID)}:\\
		\item\code{+}:\\
	\end{itemize}
\end{itemize}

\subsubsection{View::Templates::QuizController}
\begin{itemize}
