...
=======
Nella realizzazione del sistema Quizzipedia verrà adottato il design pattern Model View Presenter.
\begin{center}
	\includegraphics[scale=0.6]{../images/IMMAGINE DA INSERIRE}
\end{center}
\begin {itemize}
...
\item\textbf{Presenter}: Inoltra le richieste della View al Model
>>>>>>> origin/master
	\end {itemize}
...
<<<<<<< HEAD
	...
	\begin{itemize}
		...
		\item E' responsabile della traduzione delle domande dal formato QML a formato HTML visualizzabile da browser.  Tale funzionalità viene richiesta al momento del caricamento da database di una domanda se non è stata preventivamente tradotta, o una volta per tutte al momento del salvataggio di una domanda.
...
\begin{itemize}
	\item Conosce i riferimenti alle altre due componenti. Il Presenter è l'unica componente
che conosce entrambe le altre e facendo da singolo tramite tra Model e View permette il loro totale disaccoppiamento;
	...
	\item Al momento della creazione di una nuova domanda esegue il \emph{parsing} del codice QML tramite un componente chiamato \emph{Parser}. Se la domanda è sintatticamente corretta può richiedere al Model di salvarla nel database.
	\item Possiede dei gestori che possano modificare l'aspetto del View in reazione all'interazione dell'utente o ai dati ricevuti dal Model. Al suo interno il Presenter contiene delle classi che in risposta ad un evento, quale l'interazione dell'utente con il View o il ricevimento di una risposta dal Model, modificano l'aspetto della GUI presentata.
\end{itemize}
\newpage

=======
	\subsection{Descrizione del componente View}
	La componente View è un'interfaccia che visualizza i dati del Model e inoltra i comandi dell'utente (o gli eventi da esso generati) al Presenter che si occuperà di gestire tali richieste sui dati; la View si occupa quindi solamente della rappresentazione grafica dei dati e non ha alcun contatto diretto con essi.
	\subsection{Descrizione del componente Presenter}
>>>>>>> origin/master
